...
class CreateChartDataPt24hView < ActiveRecord::Migration[8.0]
	def up
		execute <<-SQL.squish
            CREATE OR REPLACE VIEW chart_data_pt24h_views AS
                WITH numbered_data AS (
                    ...
                    FROM chart_datas
                    WHERE label >= NOW() - INTERVAL '24 hours'
                ), grouped_data AS (
                    SELECT
                    worker_id,
                    FLOOR((row_num - 1) / 24) as group_num,
                    MIN(label) as group_start_time,
                    AVG(data) as avg_data,
                    COUNT(*) as data_points
                    FROM numbered_data
                    GROUP BY worker_id, FLOOR((row_num - 1) / 24)
                )
                SELECT
